...
def cleanup_old_google_sheet_rows(worksheet):
    ...
    try:
        ...
        for i, record in enumerate(all_records, start=2):
            checkout_data_str = record.get('Check Out Date', '')
            
            if checkout_data_str:
                try:
                    # Parse the date (assuming format is DD/MM/YYYY)
                    created_at = datetime.strptime(checkout_data_str, "%d/%m/%Y")
                    created_at = pytz.timezone('Asia/Kuala_Lumpur').localize(created_at)
                    
                    # Calculate the difference
                    days_diff = (current_date - created_at).days
                    
                    # If more than 1 day old, mark for deletion
                    if days_diff > 1:
                        rows_to_delete.append(i)
                        print(f"\nMarking row {i} for deletion - Created: {checkout_data_str}, Days old: {days_diff} \n{record}")
                        
                except ValueError as e:
                    print(f"Error parsing date '{checkout_data_str}' in row {i}: {e}")
